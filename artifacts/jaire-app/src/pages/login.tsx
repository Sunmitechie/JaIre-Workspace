...
import { loginWithSocial, getConnectedUser } from "@/lib/web3auth";

const BASE_URL = import.meta.env.BASE_URL?.replace(/\/$/, "") || "";

type Step = "checking" | "choose" | "signing" | "wallet" | "done";
...
export default function Login() {
  ...
  // On mount: check for existing session or show login buttons
  useEffect(() => {
    let cancelled = false;
    (async () => {
      // Already logged in — go straight to app
      if (isLoggedIn()) { setLocation("/baire"); return; }

      // Check for a live Web3Auth session (e.g. page refresh)
      try {
  ...
  const handleGoogle = async () => {
    ...
    setError(null);
    try {
      // Popup mode — resolves when the popup closes, no page redirect
          ...
          {step !== "choose" ? (
              ...
              <div className="text-center">
                ...
                {step === "signing" && (
                  <p className="text-xs text-muted-foreground/60 mt-2">
                    A sign-in window has opened — complete it there.
                  </p>
                )}
              </div>
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full flex items-center gap-4 px-5 py-4 rounded-xl bg-white/4 hover:bg-white/8 border border-white/8 hover:border-white/16 transition-all duration-200 text-left group"
              >
              ...
              {/* Coming soon — Twitter / Apple */}
              {[
                { label: "Continue with X (Twitter)", icon: (
                ...
                <button
                  key={p.label}
                  disabled
                  className="w-full flex items-center gap-4 px-5 py-4 rounded-xl bg-white/4 border border-white/8 opacity-40 cursor-not-allowed text-left"
                >
              ...
              <div className="space-y-2">
                <input
                  ...
                  disabled
                  className="w-full h-12 px-4 rounded-xl text-sm bg-white/4 border border-white/8 opacity-40 cursor-not-allowed placeholder:text-muted-foreground/50"
                />
                <button
                  disabled
                  className="w-full flex items-center gap-4 px-5 py-4 rounded-xl bg-primary/8 border border-primary/20 opacity-40 cursor-not-allowed text-left"
                >
                  ...
                  <span className="ml-auto text-xs text-primary/60">Soon</span>
                </button>
              </div>
            </div>
          )}
